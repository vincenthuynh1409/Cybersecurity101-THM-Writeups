...
Now, let's use `wget` to download the file using the 10.67.136.43 address and the name of the file. Remember, because the python3 server is running port 8000, you will need to specify this within your wget command. For example:

An example wget command of a web server running on port 8000

```shell-session
tryhackme@mymachine:~# wget http://10.67.136.43:8000/myfile
```
...
```shell-session
tryhackme@linux3:/tmp# wget http://10.67.136.43:8000/file
...
![](https://cdn-images.tryhackme.com/user-uploads/5c549500924ec576f953d9fc/room-content/14de6e0470d50317f3b24f4f9aa9297a.png)  
...
1. create TWO terminals (make sure they both are connected to `tryhackme@linux3`)
...
Q3) *Download the file http://10.67.136.43:8000/.flag.txt (opens in new tab) onto the TryHackMe AttackBox. Remember, you will need to do this in a new terminal.*

*What are the contents?*

1. right click on the embedded link that looks like: "http//10.67.136.43:8000/.flag.txt (opens in new tab)" > press "copy link address"
2. on the second terminal, download the file by typing: `wget [paste link address here]`
...
*Q4) Locate the process that is running on the deployed instance (10.67.176.119). What flag is given?*
...
This can be confusing to begin with, which is why there are some great resources such as the online "[Crontab Generator(opens in new tab)](https://crontab-generator.org/)" that allows you to use a friendly application to generate your formatting for you! As well as the site "[Cron Guru(opens in new tab)](https://crontab.guru/)"!
...
*Q2) When will the crontab on the deployed instance (10.66.132.175) run?*
...
1. type `cd /var/log` to access the logs on the machine
...
1. type `cd apache2` to change the directory to `apache2` in order to see that specific log
2. then `ls` to display the contents of that directory
...
![[Pasted image 20260904155643.png]]
